...
Poi genera il JSON completo.`
  ...
  const html = agentEmail({
    ...
    sections: [
      ...
      ...(brief.temi?.length ? [{ type: 'topics', label: `Tutti i temi (${brief.temi.length})`, topics: brief.temi }] : []),
      ...(brief.note_editoriali ? [{ type: 'alert', text: brief.note_editoriali, type: 'info' }] : []),
      { type: 'narrative', label: null, text: `<table cellpadding="0" cellspacing="0" style="margin:0 auto"><tr><td style="padding-right:12px"><a href="${selectUrl}" style="display:inline-block;background:#C8A97A;color:#1A1A1A;font-family:'Courier New',monospace;font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;padding:16px 40px;text-decoration:none">Seleziona i temi →</a></td></tr></table>`, dark: true },
    ]
  });.1
// Web search libero su fonti certificate → verifica con Drive → brief + approvazione
...
<table width="620" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;background:#F5F2EB">
  ...
  <tr><td style="padding:20px 28px;background:#F0EBE1;border-top:3px solid #1A1A1A;border-bottom:2px solid #C8A97A">
    ...
    <div style="font-family:'Courier New',monospace;font-size:9px;color:#8E6B33;margin-bottom:8px;text-transform:uppercase">Sezione → ${brief.raccomandazione.sezione} · ${brief.raccomandazione.angolo_editoriale}</div>
    ...
    ${brief.raccomandazione.dati_ancora?.length ? `<div style="font-family:'Courier New',monospace;font-size:9px;color:#777066;margin-top:10px">Dati da sviluppare: ${brief.raccomandazione.dati_ancora.join(' · ')}</div>` : ''}
  </td></tr>` : ''}
  ...
  <tr><td style="padding:12px 28px;background:#EDE9E0">
    <div style="font-family:'Courier New',monospace;font-size:8px;color:#9A9690">Scout Agent v2.1 · Valore Atteso · ${oggi}</div>
  </td></tr>
