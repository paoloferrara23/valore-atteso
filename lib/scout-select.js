...
// NIENTE require('@supabase/supabase-js') qui: i moduli in lib/ non risolvono
// i pacchetti npm delle function in api/. createClient viene iniettato dal
// chiamante api/run-agent.js; il client resta a livello di modulo, assegnato
...
function parseSelection(value, options) {
  if (value?.type === 'custom') {
    ...
    return {
      ...
      topic: {
        custom: text,
        titolo: text,
        sommario: 'Tema indicato direttamente dall editore.'
      }
    };
  }
  if (value?.type === 'scout') return parseChoice(value.idx, options);
  return null;
}
...
module.exports = async function handler(req, res, createClient) {
  if (!supabase) {
    if (typeof createClient !== 'function') return res.status(500).json({ error: 'createClient non iniettato dal chiamante' });
  ...
  try {
    ...
    if (req.method === 'POST') {
      const submitted = req.body?.selections;
      const bilancio = submitted
        ? parseSelection(submitted.bilancio, options.bilancio || [])
        : parseChoice(req.body?.bilancio, options.bilancio || []);
      const deal = submitted
        ? parseSelection(submitted.deal, options.deal || [])
        : parseChoice(req.body?.deal, options.deal || []);
      const metrica = submitted
        ? parseSelection(submitted.metrica, options.metrica || [])
        : parseChoice(req.body?.metrica, options.metrica || []);
