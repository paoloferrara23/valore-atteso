...
// i pacchetti npm installati per le function in api/ (il pacchetto finisce in
// api/node_modules, irraggiungibile da /var/task/lib/). Il costruttore
// createClient viene iniettato dal chiamante api/run-agent.js — stessa
// convenzione degli altri lib del repo (build-html, sponsor-edition-data).
function getSupabase(createClient) {
  if (typeof createClient !== 'function') throw new Error('createClient non iniettato dal chiamante');
...
module.exports = async function bilanciApproval(req, res, createClient) {
  ...
  try {
    const supabase = getSupabase(createClient);
