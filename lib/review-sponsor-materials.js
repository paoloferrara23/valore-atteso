...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabaseRequest(
      `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&select=id,company,contact_name,email,status,terms_version,terms_accepted_at`,
    ...
    if (!request.terms_accepted_at || request.terms_version !== TERMS_VERSION) {
      return res.status(409).json({ ok: false, error: 'Condizioni aggiornate non ancora accettate' });
    }
      ...
      await sendGmail({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;color:#1C1914;line-height:1.6">
            ...
            <p>La disponibilità selezionata resta riservata. Riceverai una preview prima di effettuare il pagamento.</p>
            <p>Nessuna pubblicazione partirà senza approvazione della preview e conferma manuale del pagamento.</p>
          </div>
        `
      });
    } else {
