...
function contentPreflight(edition) {
  ...
  // Una contraddizione confermata blocca l'invio; se non è mai stato eseguito, warning.
  ...
  if (!fc || !fc.checked_at) {
    ...
    contradictions.forEach(c => {
      push('error', 'factcheck_contradiction',
        `Fact-check ${c.section || ''}: "${String(c.claim || '').slice(0, 90)}"${c.correct ? ` → corretto: ${c.correct}` : ''}`);
    });
