...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabaseRequest(
      `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&select=id,company,contact_name,email,token,status,selected_slot_id,materials_status,payment_status`,
