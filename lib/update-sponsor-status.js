...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabaseRequest(
      `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&select=id,company,contact_name,email,status,selected_slot_id,materials_status,payment_status,preview_status,terms_version,terms_accepted_at`,
    ...
    if (action === 'mark_paid') {
      if (request.preview_status !== 'approved') {
        return res.status(409).json({ ok: false, error: 'La preview deve essere approvata prima del pagamento' });
      }
      ...
      await sendGmail({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;color:#1C1914;line-height:1.6">
            ...
            <p>Lo slot è ora confermato. Valore Atteso procederà con la programmazione editoriale concordata.</p>
          </div>
        `
      });
    } else {
      if (request.payment_status !== 'received' || request.materials_status !== 'approved' || request.preview_status !== 'approved') {
        return res.status(409).json({ ok: false, error: 'Pagamento, materiali o preview non ancora approvati' });
      }
      await supabaseRequest(`/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(request.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Prefer: 'return=minimal' },
