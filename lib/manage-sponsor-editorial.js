...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'approve_preview') {
      ...
      if (!requestId) return res.status(401).json({ ok: false, error: 'Sessione privata non valida o scaduta' });
      const rows = await supabaseRequest(
        `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&preview_status=eq.sent&select=id,company,terms_version,terms_accepted_at`,
        { headers: { Accept: 'application/json' } }
      );
      const request = rows && rows[0];
      if (!request) return res.status(404).json({ ok: false, error: 'Preview non disponibile' });
      if (!request.terms_accepted_at || request.terms_version !== TERMS_VERSION || body.publication_authorized !== true) {
    ...
    const rows = await supabaseRequest(
      `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&select=id,company,contact_name,email,status,materials_status,payment_status,edition_id,preview_status,terms_version,terms_accepted_at`,
      { headers: { Accept: 'application/json' } }
    );
    const request = rows && rows[0];
    ...
    if (action === 'update_assets') {
      ...
      if (!headline || !sponsorBody || !ctaUrl) {
        return res.status(400).json({ ok: false, error: 'Headline, testo e CTA sono obbligatori' });
      }
      try {
      ...
      if (!assets || !assets[0]) {
        return res.status(409).json({ ok: false, error: 'Materiali sponsor non disponibili' });
      }
      await supabaseRequest(`/rest/v1/sponsor_assets?request_id=eq.${encodeURIComponent(request.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Prefer: 'return=minimal' },
        body: JSON.stringify({ headline, body: sponsorBody, cta_url: ctaUrl })
      });
      await supabaseRequest(`/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(request.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Prefer: 'return=minimal' },
        body: JSON.stringify({
          materials_status: 'uploaded',
    ...
    if (action === 'send_preview') {
      if (!request.terms_accepted_at || request.terms_version !== TERMS_VERSION) {
        return res.status(409).json({ ok: false, error: 'Condizioni di sponsorizzazione non accettate' });
      }
      if (request.materials_status !== 'approved') {
        return res.status(409).json({ ok: false, error: 'Materiali non ancora approvati' });
      }
      ...
      await sendGmail({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;color:#1C1914;line-height:1.6">
            ...
            <p>puoi vedere e approvare la menzione pubblicitaria di <strong>${escapeHtml(request.company)}</strong> dal link privato:</p>
            <p>La preview riguarda esclusivamente la menzione sponsor. I contenuti editoriali della newsletter restano sotto l'esclusiva responsabilità di Valore Atteso.</p>
