...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabaseRequest(
      `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&select=id,company,status,payment_status`,
    ...
    if (request.payment_status !== 'received') {
      return res.status(403).json({
        ok: false,
        error: 'Il caricamento materiali sarà disponibile dopo la conferma manuale del pagamento'
      });
    }
