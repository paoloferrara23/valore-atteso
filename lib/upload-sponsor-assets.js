...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!requestId) {
      return res.status(401).json({ ok: false, error: 'Sessione privata non valida o scaduta' });
    }
    if (!headline || !sponsorBody || !ctaUrl || !logoData) {
    ...
    const rows = await supabaseRequest(
      `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&select=id,company,status,selected_slot_id,terms_version,terms_accepted_at`,
    ...
    if (!request.selected_slot_id || !request.terms_accepted_at || request.terms_version !== TERMS_VERSION) {
      return res.status(403).json({
        ok: false,
        error: 'Seleziona prima lo slot e accetta le condizioni'
      });
    }
