...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      catalog: SLOT_TYPES,
      slots: publicSlots,
      legal: { terms_version: TERMS_VERSION },
      payment: request.preview_status === 'approved' ? {
        bank_transfer: process.env.SPONSOR_IBAN ? {
          ...
          reference: `Sponsorizzazione Valore Atteso - ${request.company} - ${request.scheduled_date || request.id.slice(0, 8)}`
        } : null,
