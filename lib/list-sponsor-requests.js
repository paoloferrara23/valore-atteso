...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const editions = await supabaseRequest(
      '/rest/v1/editions?select=id,num,title,date,published,sent_count&sent_count=is.null&order=num.desc&limit=20',
      { headers: { Accept: 'application/json' } }
    );

    return res.status(200).json({ ok: true, requests, editions: editions || [] });
  } catch (error) {
