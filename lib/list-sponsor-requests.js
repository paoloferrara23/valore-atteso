...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabaseRequest(
      '/rest/v1/sponsor_requests?select=id,company,contact_name,email,format,notes,status,created_at,approved_at,paid_at,scheduled_date,amount,slot_type,selected_slot_id,materials_status,materials_approved_at,payment_status,sponsor_assets(id,logo_url,headline,body,cta_url,uploaded_at)&order=created_at.desc&limit=100',
