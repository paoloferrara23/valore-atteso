...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const previewEditions = await supabaseRequest(
      '/rest/v1/editions?select=id,num,title,date,published,sent_count&published=eq.true&order=created_at.desc&limit=1',
      { headers: { Accept: 'application/json' } }
    );
    ...
    return res.status(200).json({
      ...
      requests,
      editions: editions || [],
      preview_edition: previewEditions && previewEditions[0] ? previewEditions[0] : null
    });
  } catch (error) {
