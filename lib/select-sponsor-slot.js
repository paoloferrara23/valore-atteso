...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!slotId) {
      return res.status(400).json({ ok: false, error: 'Lo slot è obbligatorio' });
    }

    await ensureSponsorSlots();
    const requestRows = await supabaseRequest(
      `/rest/v1/sponsor_requests?id=eq.${encodeURIComponent(requestId)}&select=id,status,selected_slot_id,terms_version,terms_accepted_at`,
