...
// Su Vercel il token GitHub e in GH_TOKEN (vedi CLAUDE.md); teniamo GITHUB_TOKEN come fallback.
...
module.exports = async function handler(req, res) {
  ...
  if (!GITHUB_TOKEN) {
    return res.status(500).json({ error: 'Token GitHub mancante su Vercel (env var GH_TOKEN). Configuralo e redeploy.' });
  }
  ...
  try {
    ...
    if (r.status === 204) {
      ...
      return res.status(500).json({ error: 'Token GitHub non valido o scaduto (GH_TOKEN). Rigenera un PAT con scope workflow e aggiornalo su Vercel.' });
    } else {
