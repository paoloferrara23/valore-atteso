...
function buildHtml(edition) {
  ...
  function renderKpiTable(kpiRows = [], fonte = '') {
    ...
    const rows = kpiRows.map(k => `
      <tr style="border-bottom:1px solid #D0CBC0;">
        <td style="padding:5px 0;color:#9A9690;font-family:'Courier New',monospace;font-size:10px;">
          ${esc(k.key)}
          ${k.sub ? `<div style="font-family:'Courier New',monospace;font-size:7px;color:#C4BFB8;margin-top:1px;">${esc(k.sub)}</div>` : ''}
        </td>
  ...
  function renderSection(sec, idx) {
    ...
    // Supporta sia kpi_rows (legacy) che kpis (nuovo formato Supabase)
    const kpiRowsData = sec.kpi_rows?.length
      ? sec.kpi_rows
      : (sec.kpis?.length
          ? sec.kpis.map(k => ({ key: k.label, value: k.value, sub: k.sub, color: k.color, stimato: k.stimato }))
          : []);

    const rightCol = kpiRowsData.length
      ? renderKpiTable(kpiRowsData, sec.fonte) + insightBox + (sec.risk_box ? `
...
<table width="640" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;width:100%;">
  ...
  <tr><td style="background:#1A1A1A;padding:32px 28px 28px;">
    <div style="font-family:'Courier New',monospace;font-size:8px;letter-spacing:.16em;color:#D4A017;text-transform:uppercase;margin-bottom:12px;">— Questa settimana</div>
